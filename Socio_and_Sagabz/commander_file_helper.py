...
def generate_groups(df, averages, categories=constants.PROFESSIONAL_CATEGORIES):
    ...
    split_bar_graphs(
        ...
        ["גברים", "נשים"],
        averages,
        "גרף השוואה בין מגדרים",
        categories,
    )

    platoon_names = df["platoon"].unique().tolist()
    platoon_names = [
        name for name in platoon_names if name != ""
    ]  # Remove empty platoon names
    platoon_groups = [
        df.loc[df["platoon"] == name, "name"].tolist() for name in platoon_names
    ]
    split_bar_graphs(
        platoon_groups, platoon_names, averages, "גרף השוואה בין מחלקות", categories
    )
    ...
    split_bar_graphs(
        [religous_names, unreligous_names],
        ["דתיים", "חילוניים"],
        averages,
        "גרף השוואה בין דתיים לחילוניים",
        categories,
    )
    ...
    background_names = [
        name for name in background_names if name != ""
    ]  # Remove empty background names
    background_groups = [
        df.loc[df["background"] == name, "name"].tolist() for name in background_names
    ]
    split_bar_graphs(
        background_groups,
        background_names,
        averages,
        "גרף השוואה בין רקעים אקדמיים",
        categories,
    )
...
def average_compare_graph(averages, stds, categories=constants.PROFESSIONAL_CATEGORIES):
    ...
    last_avg_values = [
        averages[avg_person][-2].get(category, 0) for category in categories
    ]
    ...
    for bar, value in zip(bars, avg_values):
        ax.text(
            bar.get_width(),
            bar.get_y() + bar.get_height() - bar.get_height() / 5,
            f"{value:.2f}",
            va="center",
            fontsize=constants.SMALL_FONTSIZE,
        )
    ...
    avg_patch = plt.Line2D(
        [0], [0], color="darkgreen", lw=4, label="ממוצע מחזורי"[::-1]
    )
    overall_patch = plt.Line2D(
        [0], [0], color="lightgreen", lw=4, label="ממוצע קודם"[::-1]
    )
    ax.legend(
        handles=[avg_patch, overall_patch],
        fontsize=constants.SMALL_FONTSIZE,
        loc="upper left",
    )
    ...
    ax.tick_params(axis="x", labelsize=constants.SMALL_FONTSIZE)
    plt.title(
        "ממוצעים מחזוריים בהשוואה לסמסטר קודם"[::-1], fontsize=constants.SMALL_FONTSIZE
    )
    plt.tight_layout()
    plt.show()


def split_bar_graphs(
    ...
    group_avg_values = [
        [
            np.mean([averages[g][-1].get(category, 0) for g in group])
        ...
        for bar, value in zip(bars, avg_value):
            if categories == constants.PROFESSIONAL_CATEGORIES:
                ax.text(
                    ...
                    va="bottom",
                    fontsize=constants.SMALL_FONTSIZE
                    / math.pow(len(groups), 1 / 2),  # Scale down font size
                )
            else:
                ax.text(
                    bar.get_x() + bar.get_width() / 2,
    ...
    ax.set_xticklabels(
        [
            constants.CATEGORY_NAME_DICT[categories[j]][::-1]
    ...
    ax.tick_params(
        axis="y", labelsize=constants.SMALL_FONTSIZE * 0.8
    )  # Scale down font size
    ax.legend(
        fontsize=constants.SMALL_FONTSIZE * 0.8, loc="lower right"
    )  # Scale down font size

    # Title and layout
    ax.set_title(
        title[::-1], fontsize=constants.SMALL_FONTSIZE * 0.8
    )  # Scale down font size
    plt.tight_layout()
    plt.show()
...
def correlation_heatmap(df, cols=constants.PROFESSIONAL_CATEGORIES):
    ...
    for i in range(len(corr_arr)):
        for j in range(len(corr_arr)):
            plt.text(
                j, i, f"{corr_arr[i, j]:.2f}", ha="center", va="center", color="black"
            )
